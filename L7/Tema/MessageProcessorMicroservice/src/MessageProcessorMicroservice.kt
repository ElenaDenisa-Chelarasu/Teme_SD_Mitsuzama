...
class MessageProcessorMicroservice {
    ...
    private fun predicate(it: String): Boolean{
        val message = Message.deserialize(it.toByteArray())
        val len = message.body.length
        return message.body.substring(len-4).toInt()>6000
    }
    ...
    private fun sendProcessedMessages() {
        try {
            ...
            Observable.fromIterable(messageQueue)
                .sorted { m1:Message, m2: Message -> m1.timestamp.compareTo(m2.timestamp) }
                .subscribeBy(
                onNext = {
                    println(it.toString())
