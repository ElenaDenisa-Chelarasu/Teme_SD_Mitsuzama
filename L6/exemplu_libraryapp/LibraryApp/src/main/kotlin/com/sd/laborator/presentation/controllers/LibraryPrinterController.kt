...
class LibraryPrinterController {
    ...
    @RabbitListener(queues = ["\${cache.rabbitmq.queueFromCacheToLibrary}"])
    fun receiveMessageFromCache(msg: String) {
        ...
        sendMessageToCache(response)
    }

    fun sendMessageToCache(msg: String) {
        println("Response for Cache: \"$msg\"")
        _amqpTemplate.convertAndSend(_rabbitMqComponent.getExchange(), _rabbitMqComponent.getRoutingKeyFromLibraryToCache(), msg)
    }
