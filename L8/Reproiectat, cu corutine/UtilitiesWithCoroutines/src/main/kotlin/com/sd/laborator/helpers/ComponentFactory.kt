...
class ComponentFactory(private val name: String, private val id: Int) {
    ...
    fun createQuestionListener(
        answerGenerator: (Message) -> String?,
        responseProcessor: (Message) -> Unit
    ): AbstractQuestionAndResponseListener {
    ...
    fun createCommandable(port: Int, responseProcessor: (Message) -> Unit): AbstractCommandableComponent {
        return object : AbstractCommandableComponent(name, id, port, messageManagerCommandsSocket) {
            override fun receiveResponse(response: Message) = responseProcessor(response)
        }
    }
}