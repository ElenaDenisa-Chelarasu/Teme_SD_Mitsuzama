...
import com.sd.laborator.abstractClasses.AbstractQuestionListener
import com.sd.laborator.interfaces.IQuestionListener
...
class StudentMicroserviceV2: IQuestionListener {
    private val commandableThread: Commandable
    private var questionDatabase: MutableList<Pair<String, String>>
    private val questionListener: AbstractQuestionListener
    ...
    init {
        ...
        // Instantiere factory
        val factory = ComponentFactory("StudentMicroservice")

        // Instantiere thread care se ocupa cu comunicarea cu GUI-ul
        commandableThread=factory.createCommandable(STUDENT_PORT)

        // Instantiere thread care asculta intrebari si trimite raspunsuri
        questionListener = factory.createQuestionListener(this::answerGenerator, this::responseProcessor)
    }

    override fun answerGenerator(question: Message): String? {
        return questionDatabase.firstOrNull { it.first==question.content }?.second
    }

    override fun responseProcessor(response: Message)=println("Sunt student, nu ma intereseaza raspunsurile!")

    override fun run(){
        commandableThread.start()
        questionListener.start()
    }
}

fun main(args: Array<String>) {
    val studentMicroservice = StudentMicroserviceV2()//StudentMicroservice()
